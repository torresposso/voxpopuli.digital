/**
 * Modified for multi-conditionals.
 */
jQuery( document ).ready( function( $ ) {
	...
	function _shouldShow(elm, requiredValue, elmValue) {
		var checkedValues;
		var loopIndex = 0;
		var shouldShow = true;
		...
		} else if (typeof elmValue === 'undefined') {
			...
			if ( dependants.length > 0 ) {
				...
				dependants.each( function( i, e ) {
					...
					if ( 'undefined' !== typeof currentFieldName && '' !== currentFieldName && $.inArray( currentFieldName, dependantsSeen ) < 0 ) {
						...
						const multiDepend = current.data('conditional-id');
						let shouldShow    = true;

						if (Array.isArray(multiDepend) && multiDepend[0].key) {
							for (i in multiDepend) {
								const dependsOn = $('[name=' + multiDepend[i].key + ']');
								let expected;
								if (typeof multiDepend[i].value !== 'undefined') {
									expected = multiDepend[i].value;
								}

								if (!_shouldShow(dependsOn, expected)) {
									shouldShow = false;
									break;
								}
							}
							...
							shouldShow = _shouldShow(elm, requiredValue, CMB2ConditionalsStringToUnicode( evt.currentTarget.value ));	
						}
	...
	let dependantData;
	function getDependants(field, context) {

		if (!dependantData) {
			dependantData = {};

			$('[data-conditional-id]', context).each(function() {

				const element     = $(this);
				let conditions    = $(this).data('conditional-id');
				const expectedVal = $(this).data('conditional-value');
				...
				conditions.forEach(item => {
					if (!dependantData[item.key]) {
						dependantData[item.key] = [];
					}

					dependantData[item.key].push(element[0]);
				});
			});
		}
		...
		// const dependEles = dependantData.reduce((acc, item) => {
		// 	if (item.key === field) {
		// 		acc.push(item.ele);
		// 	}
		// 	return acc;
	...
	function CMB2ConditionalsFindDependants( fieldName, elm, context ) {
		...
		// Else within the whole form.
			...
			dependants = getDependants(fieldName, context);
