...
class WPCLICommands extends \WP_CLI_Command {
	...
	public function list_predefined() {
		...
		foreach ( $this->ocdi->import_files as $index => $import_file ) {
			WP_CLI::log( sprintf(
				'%s -> %s [content: %s, widgets: %s, customizer: %s, redux: %s]',
				$index,
				empty( $import_file['import_file_name'] ) ? $index : $import_file['import_file_name'],
	 ...
	 *
	 * [--predefined=<index>]
	 * : The index or slug of the predefined demo imports (use the 'list_predefined' command to check the predefined demo imports)
	 */
	 ...
	 *
	 * @param string $predefined_index Index or slug of a OCDI predefined demo import.
	 */
	private function import_predefined( $predefined_index ) {
		if ( is_numeric( $predefined_index ) ) {
			...
			$keys = array_keys( $this->ocdi->import_files );
			if ( isset( $keys[ $predefined_index ] ) ) {
				$predefined_index = $keys[ $predefined_index ];
			}
		}
