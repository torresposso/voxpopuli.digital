...
<aside {{ $attributes->merge(['class' => 'drawer-side z-50']) }} aria-label="{{ __('Menú de navegación lateral', 'voxpopuli') }}">
  ...
  <div class="bg-base-200 min-h-full w-full max-w-full sm:w-[460px] sm:max-w-none flex flex-col justify-between border-r border-base-300 shadow-2xl relative overflow-hidden">
    ...
    <div class="flex flex-col flex-1 relative z-10 overflow-y-auto">
      ...
      {{-- ⚡ Bolt: Cache expensive queries and data extraction in globally rendered layout components to prevent execution on every page load --}}
      @php
        $drawer_feat_post_data = \Illuminate\Support\Facades\Cache::remember('voxpopuli_drawer_featured_post', 3600, function () {
      ...
      @if($drawer_feat_post_data)
        <div class="px-6 py-6 bg-base-100/30 border-b border-base-300/40 shrink-0">
          ...
          <a href="{{ $drawer_feat_post_data['link'] }}" class="drawer-featured-card flex gap-4 p-3 bg-base-100 rounded-xl overflow-hidden hover:no-underline group">
            @if($drawer_feat_post_data['thumb'])
              <div class="w-20 h-20 rounded-lg overflow-hidden shrink-0 bg-base-200 relative">
                <img src="{{ $drawer_feat_post_data['thumb'] }}" alt="{{ esc_attr($drawer_feat_post_data['title']) }}" class="w-full h-full object-cover filter grayscale group-hover:grayscale-0 transition-all duration-500">
              </div>
            @else
            ...
            <div class="flex flex-col justify-center min-w-0">
              <span class="font-sans text-[8px] font-extrabold uppercase tracking-widest text-secondary mb-1">
                {{ $drawer_feat_post_data['cat_name'] }}
              </span>
