...
<div class="reading-progress-bar fixed top-0 left-0 h-1.5 bg-secondary z-50 shadow-[0_0_10px_rgba(239,133,25,0.5)] origin-left" style="width: 0%; transform: scaleX(0);"></div>

<article @php(post_class('h-entry w-full relative'))>
  @if ($img)
    {{-- Full-bleed Hero Header with Dark Overlay & Texture --}}
    <header class="group relative w-full min-h-[50vh] md:min-h-[65vh] flex items-end overflow-hidden mb-10 md:mb-14 bg-base-200 border-b border-base-300 pt-28 pb-8 md:pb-12">
      ...
      <div class="absolute inset-0 w-full h-full">
        <img src="{{ $img['url'] }}" 
             ...
             loading="eager"
             fetchpriority="high" />
        <div class="absolute inset-0 bg-black/60 mix-blend-multiply transition-colors duration-700 group-hover:bg-black/40"></div>
        <div class="absolute inset-0 noise-overlay opacity-30 mix-blend-overlay pointer-events-none"></div>
      </div>
      ...
      <div class="relative z-10 w-full max-w-3xl mx-auto px-4 md:px-0 text-white">
        ...
        <h1 class="p-name font-display text-4xl md:text-5xl lg:text-6xl font-extrabold text-white leading-[1.05] tracking-tighter mb-6 animate-fade-in-up drop-shadow-lg flex items-start gap-3">
          <span class="text-secondary select-none font-sans font-black tracking-widest mt-1 opacity-90">.:</span>
          <span>{!! $title !!}</span>
        </h1>
  ...
  @else
    ...
    <header class="max-w-3xl mx-auto px-6 md:px-10 pt-12 md:pt-16 pb-8 md:pb-10 mb-8 md:mb-12 bg-base-100 border border-base-300/50 rounded-sm shadow-sm relative overflow-hidden">
      {{-- Primary Top Bar --}}
      ...
      <h1 class="p-name font-display text-4xl md:text-5xl lg:text-6xl font-extrabold text-base-content leading-[1.05] tracking-tighter mb-6 animate-fade-in-up flex items-start gap-3">
  ...
  <div class="max-w-3xl mx-auto px-4 md:px-0 pb-12">
    @if ($img && $img['caption'])
      <div class="font-sans text-xs text-muted/80 mb-10 -mt-6 px-6 py-2.5 border-l-2 border-secondary/50 italic text-left bg-base-200/40 rounded-r-md">
        {!! $img['caption'] !!}
      </div>
    @endif
