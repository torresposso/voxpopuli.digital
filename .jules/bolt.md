...
## 2024-06-27 - [Filter Cascade Bottlenecks in Bulk Processing]
**Learning:** In bulk rendering contexts (like processing thousands of posts for a sitemap), using standard WP formatting functions like `get_the_modified_date()` creates a massive architectural bottleneck. Each call triggers multiple filters (`get_the_modified_time` and `get_the_modified_date`) which cascade and compound CPU overhead unnecessarily when only raw data is needed.
**Action:** When building bulk loops (e.g. WP-CLI commands or sitemaps), fetch the primed post object via `get_post($id)` and access raw database properties directly (e.g. `substr($post->post_modified, 0, 10)`) to completely bypass the WordPress filter cascade.
