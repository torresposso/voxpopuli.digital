...
## 2024-05-19 - [Drawer Layout N+1 Query Bottleneck]
**Learning:** Layout components (like a navigation drawer or header) that are included on every single page load can create a massive architectural bottleneck if they execute raw WordPress database queries (e.g., `get_posts`) or expensive filter cascades (`get_the_category`, `get_the_title`) unconditionally. Because they render globally, the impact of the bottleneck is multiplied across the entire application traffic.
**Action:** Always wrap database queries and expensive data extraction logic inside globally rendered layout components with a unified caching layer (e.g., `\Illuminate\Support\Facades\Cache::remember()`) to prevent the same queries from executing unconditionally on every single request, and ensure proper invalidation hooks (like `save_post`) are set up in the Service Providers.
